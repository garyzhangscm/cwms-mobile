...
class WarehouseLocationService {
  ...
  static Future<WarehouseLocation> getWarehouseLocationById(int id) async {
    ...
    if (responseString["result"] as int != 0) {
      printLongLogMessage("getWarehouseLocationById / Start to raise error with message: ${responseString["message"]}");
  ...
  static Future<WarehouseLocation> getWarehouseLocationByWarehouseIdAndName(int warehouseId, String locationName) async {
    ...
    if (responseString["result"] as int != 0) {
      printLongLogMessage("getWarehouseLocationByName / Start to raise error with message: ${responseString["message"]}");
      throw new WebAPICallException(responseString["result"].toString() + ":" + responseString["message"]);
    }
  ...
  static Future<WarehouseLocation> getWarehouseLocationByCode(String locationCode) async {
    ...
    if (responseString["result"] as int != 0) {
      printLongLogMessage("getWarehouseLocationByCode / Start to raise error with message: ${responseString["message"]}");
      throw new WebAPICallException(responseString["result"].toString() + ":" + responseString["message"]);
    }
