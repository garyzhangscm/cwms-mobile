...
showBlockedErrorDialog(BuildContext context, String message) async {
  await showBlockedDialog(
      context, CWMSLocalizations.of(context).error, message);
}

showBlockedWarningDialog(BuildContext context, String message) async {
  await showBlockedDialog(
      context, CWMSLocalizations.of(context).warning, message);
}
...
Future<bool?> showYesNoDialog(BuildContext context, String title,
    String message, Function yesOnPressed, Function noOnPressed) {
  Widget okButton = TextButton(
...
Future<bool?> showYesNoCancelDialog(
  ...
  AlertDialog alert = AlertDialog(
    title: Text(title),
    content: Text(message),
    actions: [yesButton, noButton, cancelButton],
  );
...
Widget buildSingleButtonRow(BuildContext context, Widget button) {
  return Row(
      ...
      children: [
        SizedBox(width: MediaQuery.of(context).size.width * 0.9, child: button),
      ]);
}

Widget buildTwoButtonRow(BuildContext context, Widget button1, Widget button2) {
  return buildTwoButtonRowWithWidth(
      context,
      button1,
...
Widget buildTwoButtonRowWithWidth(BuildContext context, Widget button1,
    double width1, Widget button2, double width2) {
  return Row(
      ...
      verticalDirection: VerticalDirection.down,
      children: [
        SizedBox(width: width1, child: button1),
        SizedBox(width: width2, child: button2),
      ]);
}

Widget buildThreeButtonRow(
    BuildContext context, Widget button1, Widget button2, Widget button3) {
  return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      mainAxisSize: MainAxisSize.max,
      //交叉轴的布局方式，对于column来说就是水平方向的布局方式
      ...
      children: [
        SizedBox(
            width: MediaQuery.of(context).size.width * 0.3, child: button1),
        SizedBox(
            width: MediaQuery.of(context).size.width * 0.3, child: button2),
        SizedBox(
            width: MediaQuery.of(context).size.width * 0.3, child: button3),
      ]);
}
...
Widget buildFourSectionInformationRow(
...
Widget buildSingleSectionInputRow(Widget inputController) {
  return Padding(
    ...
    child:
        ...
        Row(children: <Widget>[Expanded(child: inputController)]),
  );
}
...
Widget buildTwoSectionInputRow(String name, Widget inputController,
  ...
  return Padding(
    ...
    child:
      ...
      expanded ? Expanded(child: inputController) : inputController
    ]),
  );
}

Widget buildThreeSectionInputRow(
    String name, Widget inputController1, Widget inputController2) {
  return Padding(
    padding: EdgeInsets.only(top: 5, bottom: 5),
    child:
        // confirm the location
      ...
      Expanded(child: inputController1),
      Expanded(child: inputController2)
    ]),
  );
}

Widget buildFourSectionRow(Widget inputController1, Widget inputController2,
    Widget inputController3, Widget inputController4) {
  return Padding(
    padding: EdgeInsets.only(top: 5, bottom: 5),
    child:
        // confirm the location
...
Future<String> uploadFile(XFile file, String url) async {
  ...
  Response response = await httpClient.post(url, data: fileData);
  ...
  if (responseString["result"] as int != 0) {
    printLongLogMessage(
        "uploadFile / Start to raise error with message: ${responseString["message"]}");
    throw new WebAPICallException(
        responseString["result"].toString() + ":" + responseString["message"]);
  }

  printLongLogMessage(
      "File ${file.path} uploaded! filepath: ${responseString["data"]}");
